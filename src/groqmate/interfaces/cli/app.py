...
class GroqmateApp(App):
    ...
    def _show_welcome(self) -> None:
        ...
        chat.add_message(
            "System",
            "Welcome to Groqmate! Your learning coach.\n"
            "Type 'teach me <topic>' to start a lesson.\n"
            "Commands: next, wtf, summary, quit",
            is_system=True
        )
    ...
    async def _handle_next(self) -> None:
        ...
        if not self.session.state.plan:
            chat.add_message("System", "No active lesson. Start with 'teach me <topic>'", is_system=True)
            return
        
        if self.session.is_in_quiz():
            chat.add_message("System", "Answer the quiz first to unlock the next step.", is_system=True)
            return
        
        if self.session.is_complete():
            chat.add_message("Groqmate", "Lesson complete! Type 'summary' to get your notes.", is_user=False)
            return
